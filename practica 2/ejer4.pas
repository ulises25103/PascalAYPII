...
type
    ...
    Vector200 = array[-10..10] of integer;
...
begin
    ...
    while (not l.eol()) do
      begin
        if (l.current() = n) then begin
          l.removeCurrent();
        end;
        l.next();
      end;
end;
function CargarListaOrdenada (num,max,min:integer):ListNum;
var
    num_ale, i:integer;
    l: ListNum;
    seguir:boolean;
...
procedure EliminarTodosLosElementosConOrden(var l:ListNum; n:integer);
begin
    l.reset();
...
procedure InicializarVector(var v:Vector200);
	var
		i:integer;
...
function ContarValores (l: ListNum, num: integer): Vector200;
var
i: integer;
vContador: Vector200;
begin
...
while (not l.eol()) do
begin
	vContador[l.current()] := vContador[l.current()] + 1;
	l.next();
end;
ContarValores:= vContador;
end;

writeln('el valor: ', num,'esta ', contador, ' veces');
...
begin
    ...
    write ('Dime de cuantos numeros quieres la lista: ');
    readln (num);
    listorden:= CargarListaOrdenada (num,max,min);
